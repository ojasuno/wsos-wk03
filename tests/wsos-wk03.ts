...
describe("calculator", () => {
  ...
  it('Addition',async () => {
    await program.methods.add(new anchor.BN(2), new anchor.BN(1))
    .accounts({
        calculator: calculatorPair.publicKey,
    })
    .rpc()
  ...
  //Another test step - test out subtract
  it('Subtraction',async () => {
    await program.methods.subtract(new anchor.BN(4), new anchor.BN(3))
    .accounts({
        calculator: calculatorPair.publicKey,
    })
    .rpc()
  ...
  it('Multiplication',async () => {
    await program.methods.multiply(new anchor.BN(6), new anchor.BN(5))
    .accounts({
        calculator: calculatorPair.publicKey,
    })
    .rpc()
    const account = await program.account.calculator.fetch(calculatorPair.publicKey)
    expect(account.result).to.eql(new anchor.BN(30))
  })
